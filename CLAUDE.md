...
### Meatball ignores its connection-approval spawn position (interpolated Rigidbody)

If a meatball spawns at the prefab's authored transform (and falls from it) instead of the position set in `GameManager.ApprovalCheck` via `response.Position`, **do not** assume the approval logic is wrong. NGO applies the connection-approval position with `transform.SetPositionAndRotation` (`NetworkSpawnManager.InstantiateNetworkPrefab`), which sets the **Transform** — but an **interpolated, non-kinematic Rigidbody** (`MeatballPhysicsController.Awake` sets `RigidbodyInterpolation.Interpolate`) does not reliably adopt a bare transform teleport on the spawn frame. The interpolation system restores the body's cached prefab pose, the transform follows the body, and it falls from there. The host is authoritative and gets no snapshot to self-correct, so the wrong pose is permanent and visible (an owning client partly masks it by hard-snapping to the first snapshot).

The fix lives in `MeatballPhysicsController.OnNetworkSpawn`: snap the physics body to the spawned transform once, `if (IsServer || IsOwner) { _rb.position = transform.position; _rb.rotation = transform.rotation; }`. In `OnNetworkSpawn`, `transform.position` already holds NGO's applied value while `_rb.position` may still read the stale prefab pose, so the assignment makes the physics source of truth correct. Confirm the bug is here, not in approval, by checking the `[Approval]` log: `resolved Position=` will show the correct marker value even though the body lands at the prefab transform.
